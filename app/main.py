...
def get_campaign_with_summary(campaign_id: int, current_brand: Brand = Depends(get_current_brand)):
    with Session(engine) as session:
        ...
        # -----------------------------
        # ✅ FIX: calculate unique locations from DistributionRecord
        # -----------------------------
        ...
        # -----------------------------
        # Totals (now includes locations)
        # -----------------------------
        totals = {
            # prefer accurate per-day sums of DailyActivity (safer than campaign.manufactured if you use per-day entries)
...
def add_daily_activity(campaign_id: int, data: DailyActivityCreate, current_brand: Brand = Depends(get_current_brand)):
    ...
    with Session(engine) as session:
        ...
        if existing_activity:
            # subtract old values from campaign totals (if you're keeping campaign.manufactured/distributed)
        ...
        # update campaign counters (optional — we keep per-day sums as the authoritative totals elsewhere)
        ...
        # -----------------------
        # Handle locations if provided in request
        # Important: store distribution records with a timestamp that belongs to the 'day'
        # so queries by day range will reliably return these records for that date.
        # -----------------------
        if getattr(data, "locations", None):
            day_start = datetime.combine(data.day, time.min)
            day_end = day_start + timedelta(days=1)

            # remove any existing distribution records for this campaign/day
            ...
            # create new distribution records — set distributed_at to midday of the requested day
        ...
        # -----------------------
        # Compute blockchain proof for this daily entry
        # -----------------------
        proof_obj = {
            "type": "daily_activity",
            "campaign_id": campaign_id,
        ...
        # return activity with attached locations for this day
        ...
        locations = [
            {"location_name": d.location_name, "distributed_count": d.distributed_count, "lat": d.lat, "lng": d.lng}
            for d in d_recs
        ]
...
def get_daily_activities(campaign_id: int, current_brand: Brand = Depends(get_current_brand)):
    ...
    with Session(engine) as session:
        ...
        activities = session.exec(
            select(DailyActivity).where(DailyActivity.campaign_id == campaign_id).order_by(DailyActivity.day.desc())
        ).all()
